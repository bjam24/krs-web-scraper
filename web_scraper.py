...
def scrape_krs_data(start_num: int, end_num: int) -> pd.DataFrame:
    ...
    krs_dataset = pd.DataFrame(columns=['KRS', 'OrganizationName', 'LegalForm', 'Representatives', 'RepresentativesID', 'ConnectedOrganizations'])
    ...
    for krs_num in range(start_num, end_num):
        ...
        if driver_organizations.title != 'Nie znaleziono strony | Rejestr.io':
            ...
            legal_form = driver_organizations.find_element(By.CSS_SELECTOR, "div[data-name='forma_prawna'] .val.text-normalize span").text
            ...
            representatives_id = driver_representatives.find_elements_by_css_selector('li[data-global-id]')
            ...
            for representative in representatives:
                krs_dataset_row = dict.fromkeys(['KRS', 'OrganizationName', 'LegalForm', 'Representatives', 'RepresentativesID', 'ConnectedOrganizations'])
                ...
                if link_to_connections:
                    ...
                    connections_krs = driver_connections.find_elements_by_css_selector('li[data-global-id]')
                    ...
                    krs_dataset_row['ConnectedOrganizations'] = str(dict(zip(list_of_connections, list_of_connections_krs))).replace('{', '').replace('}', '') #', '.join(map(lambda x: f"'{x}'", list_of_connections))
                else:
                    krs_dataset_row['ConnectedOrganizations'] = np.nan
